...
public class TermConditionActivity extends AppCompatActivity implements ProfileViews {
    ...
    public void onSuccessPagesAPi(PagesResponse pagesResponse) {
        try {
            ...
            {
                tv_termAndCondition.setText(Html.fromHtml(""+pagesResponse.getResult().get(2).getArabicDescription()));
            }
            if(SharedPreferencesUtils.getInstance(getApplicationContext()).getValue(Constants.Language, "").equalsIgnoreCase("en"))
            {
                tv_termAndCondition.setText(Html.fromHtml(""+pagesResponse.getResult().get(2).getDescription()));
            }
